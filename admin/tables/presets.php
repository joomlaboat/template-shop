...
/**
 * Presetslist Table class
 */
class TemplateshopTablePresets extends JTable
{
	/**
	...
	{
		parent::__construct('#__templateshop_presets', 'id', $db); 
	}	
	...
	/**
	 * Overload the store method for the Presets table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New presets. A presets created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('presets', 'TemplateshopTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_TEMPLATESHOP_PRESETS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_templateshop.presets.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_templateshop.presets.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_templateshop');
